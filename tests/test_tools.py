...
def test_deploy_image_to_test_slot_dispatches_and_polls_to_terminal(monkeypatch) -> None:
    tools, client = _registered_tools()
    monkeypatch.setattr(tools_mod, "_SLOT_OP_POLL_INTERVAL_SECONDS", 0)
    ...
    client.responses[
        ("GET", "/v1/test-slots/jobs/tank-operator/deploy-x")
    ...
    # Minimal payload: project + git_ref + slot.
    ...
    assert (
        "GET",
        "/v1/test-slots/jobs/tank-operator/deploy-x",
        None,
        None,
    ) in client.calls
